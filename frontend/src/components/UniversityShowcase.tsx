...
import { GraduationCap, Stethoscope, Briefcase, ChevronRightSquare, MapPin, Building, Globe } from "lucide-react";
...
export default function UniversityShowcase() {
  ...
  if (loading || !university) {
    return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><div className="w-8 h-8 border-4 border-brand-primary border-t-transparent rounded-full animate-spin"></div></div>;
  }
  ...
  return (
    <div className="w-full bg-white font-sans text-slate-800">
      ...
      <section className="relative w-full pt-[calc(6rem+var(--banner-height,0px))] lg:pt-[calc(8rem+var(--banner-height,0px))] pb-16 bg-[#1a237e] text-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-7 z-10">
              <div className="flex flex-wrap gap-3 mb-6">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-sm rounded-sm border border-white/20 text-[#cfb53b] text-sm font-medium">
                  <Globe className="w-4 h-4" />
                  <span>Premium Partner University</span>
                </div>
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-sm rounded-sm border border-white/20 text-white text-sm font-medium">
              ...
              <div className="text-white/80 text-lg max-w-2xl space-y-4 font-light">
            ...
            <div className="lg:col-span-5 relative">
              <div className="aspect-[4/3] w-full rounded-sm overflow-hidden shadow-2xl relative border-4 border-white/10 bg-[#111859]">
                {university.image ? (
                  <Image 
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center">
                    <Building className="w-16 h-16 mb-4 opacity-50 text-white/50" />
                    <p className="text-white/50">Upload Main Image</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {courseParagraphs.map((paragraph, idx) => {
            ...
            return (
              <div 
                key={idx}
                className="bg-slate-50 rounded-sm p-8 border border-slate-200 hover:border-[#1a237e]/30 transition-colors flex flex-col h-full"
              >
                ...
                <p className="text-slate-700 leading-relaxed mb-8 flex-grow">{mainDescription}</p>
                
                {highlights && (
                  <div className="mt-auto bg-white p-5 border border-slate-100 rounded-sm">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {images.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {images.map((img, idx) => (
                <div key={idx} className="group relative aspect-[4/3] overflow-hidden rounded-sm bg-slate-200 shadow-sm border border-slate-200">
                  <Image
            ...
            <div className="flex flex-col items-center justify-center p-12 bg-white border border-slate-200 rounded-sm text-center">
      ...
      <section className="py-20 bg-[#1a237e] text-white text-center">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <button 
            ...
            className="inline-flex items-center gap-3 px-8 py-4 bg-[#cfb53b] text-[#1a237e] rounded-sm font-bold hover:bg-white transition-colors shadow-lg"
          >
